/obj/item/reagent_containers/glass/vial
	...
	icon = 'topiastation_modules/hyposprays/icons/vials.dmi'
	icon_state = "hypovial"
	spillable = FALSE
	volume = 10
	...
	var/type_suffix = "-s"
	/// The colour of the chemical, used for hypospray overlay
	var/chem_color
...
/obj/item/reagent_containers/glass/vial/update_overlays()
    ...
    if(reagents.total_volume)
        ...
        switch(round((reagents.total_volume / volume)*100))
            ...
            if(89 to 100)
                fill_overlay = 100
        var/mutable_appearance/filling = mutable_appearance('topiastation_modules/hyposprays/icons/hypospray_fillings.dmi', "[fill_name][fill_overlay]")
...
/obj/item/reagent_containers/glass/vial/large
	...
	volume = 120
	type_suffix = "-l"
	possible_transfer_amounts = list(1, 2, 5, 10, 20, 30, 40, 50, 60, 100, 120)
